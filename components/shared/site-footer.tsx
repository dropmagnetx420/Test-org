import Link from "next/link";
import { Logo } from "@/components/shared/logo";
import { SPORTS, SITE_DESCRIPTION } from "@/lib/constants";
import type { SiteSettings } from "@/types/database";

const TwitterIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="size-4" aria-hidden>
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
  </svg>
);

const TelegramIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="size-4" aria-hidden>
    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
  </svg>
);

const DiscordIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="size-4" aria-hidden>
    <path d="M20.317 4.37a19.79 19.79 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.74 19.74 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.1 13.1 0 0 1-1.872-.892.077.077 0 0 1-.008-.128c.126-.094.252-.192.372-.291a.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.009c.12.099.246.198.373.292a.077.077 0 0 1-.006.127 12.3 12.3 0 0 1-1.873.891.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.84 19.84 0 0 0 6.002-3.03.077.077 0 0 0 .032-.055c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.028zM8.02 15.331c-1.183 0-2.157-1.086-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.211 0 2.176 1.096 2.157 2.42 0 1.332-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.086-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.332-.946 2.418-2.157 2.418z" />
  </svg>
);

export function SiteFooter({ settings }: { settings: SiteSettings | null }) {
  const socials = [
    { href: settings?.twitter_url, label: "Twitter", Icon: TwitterIcon },
    { href: settings?.telegram_url, label: "Telegram", Icon: TelegramIcon },
    { href: settings?.discord_url, label: "Discord", Icon: DiscordIcon },
  ].filter((s): s is { href: string; label: string; Icon: () => React.JSX.Element } =>
    Boolean(s.href)
  );

  return (
    <footer className="mt-20 border-t border-border/50 bg-background/60 backdrop-blur-xl">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-4 lg:px-8">
        <div className="space-y-4 md:col-span-2">
          <Logo siteName={settings?.site_name ?? "NextGen Predict"} />
          <p className="max-w-sm text-sm text-muted-foreground">
            {settings?.site_tagline || SITE_DESCRIPTION}
          </p>
          {socials.length > 0 && (
            <div className="flex items-center gap-2">
              {socials.map(({ href, label, Icon }) => (
                <a
                  key={label}
                  href={href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={label}
                  className="grid size-9 place-items-center rounded-lg border border-border/60 bg-card/60 text-muted-foreground transition-colors hover:border-primary/50 hover:text-primary"
                >
                  <Icon />
                </a>
              ))}
            </div>
          )}
        </div>

        <div className="space-y-3">
          <p className="text-sm font-semibold">Markets</p>
          <ul className="space-y-2 text-sm text-muted-foreground">
            {SPORTS.map((sport) => (
              <li key={sport.value}>
                <Link
                  href={`/markets?sport=${sport.value}`}
                  className="transition-colors hover:text-primary"
                >
                  {sport.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        <div className="space-y-3">
          <p className="text-sm font-semibold">Account</p>
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li>
              <Link href="/dashboard" className="transition-colors hover:text-primary">
                Dashboard
              </Link>
            </li>
            <li>
              <Link href="/earn" className="transition-colors hover:text-primary">
                Earn rewards
              </Link>
            </li>
            <li>
              <Link href="/wallet" className="transition-colors hover:text-primary">
                Wallet
              </Link>
            </li>
            <li>
              <Link href="/kyc" className="transition-colors hover:text-primary">
                Verification
              </Link>
            </li>
            <li>
              <Link href="/referrals" className="transition-colors hover:text-primary">
                Referrals
              </Link>
            </li>
            {settings?.support_email && (
              <li>
                <a
                  href={`mailto:${settings.support_email}`}
                  className="transition-colors hover:text-primary"
                >
                  Support
                </a>
              </li>
            )}
          </ul>
        </div>
      </div>

      <div className="border-t border-border/50">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-4 py-6 text-xs text-muted-foreground sm:flex-row sm:px-6 lg:px-8">
          <p>
            © {new Date().getFullYear()} {settings?.site_name ?? "NextGen Predict"}. All rights
            reserved.
          </p>
          <p>Prediction markets carry risk. Trade responsibly and only what you can afford.</p>
        </div>
      </div>
    </footer>
  );
}
