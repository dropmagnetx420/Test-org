-- ===========================================================================
-- Flat referral reward on a referred user's first approved deposit
--
-- Referrals already pay an ongoing percentage of trading fees
-- (credit_referral_commission). This adds a one-off flat bounty the admin can
-- set, paid to the referrer the moment their referred user's FIRST deposit is
-- approved. Set to 0 to disable. The percentage commission is untouched, so a
-- referrer can earn both.
-- ===========================================================================

alter table public.site_settings
  add column if not exists referral_signup_reward numeric(20,2) not null default 0
    check (referral_signup_reward >= 0);

comment on column public.site_settings.referral_signup_reward is
  'Flat USDG paid to the referrer on a referred user''s first approved deposit. 0 = off.';

-- ======================================================= APPROVE DEPOSIT
-- 20250107 body + a flat referral bounty in the first-deposit branch.
